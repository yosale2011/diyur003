...
def generate_gesher_file(conn, year: int, month: int, filter_name: str = None, company: str = None) -> str:
    ...
    # אם אין הגדרות ב-DB, ננסה לטעון מהקובץ כגיבוי (או אם רוצים לשלב)
    if not export_codes:
        export_codes = load_export_config()
    ...
    # הכנת cache לזמני שבת
    shabbat_cache = get_shabbat_times_cache(conn)
    minimum_wage = get_minimum_wage(conn)
    
    # שליפת עובדים עם קוד מירב
    if filter_name:
        # סינון לפי שם - זיהוי אוטומטי של המפעל
        cursor = conn.execute("""
            SELECT p.id, p.name, p.meirav_code, p.employer_id, e.code as employer_code
            FROM people p
            LEFT JOIN employers e ON p.employer_id = e.id
        ...
        # זיהוי אוטומטי של המפעל מהעובד הראשון
        if people and people[0]['employer_code']:
            company = people[0]['employer_code']
    else:
        # שליפת עובדים עם פרטי המפעל שלהם
        cursor = conn.execute("""
            SELECT p.id, p.name, p.meirav_code, e.code as employer_code
            FROM people p
            LEFT JOIN employers e ON p.employer_id = e.id
    ...
    for person in people:
        ...
        for symbol, value_tuple in export_codes.items():
            ...
            # לסוג hours - בודקים רק את הכמות (התעריף תמיד חיובי)
            # לסוג money - בודקים את הסכום (הכמות תמיד 0)
            if not options['export_zero_values']:
                if value_type.startswith('hours_') and quantity < options['min_amount']:
                    continue
                elif value_type == 'money' and rate < options['min_amount']:
    ...
    result = output.getvalue()
    print(f"Gesher export: {line_count} lines for {len(people)} employees")
    return result
...
def get_export_preview(conn, year: int, month: int, limit: int = 50) -> List[Dict]:
    ...
    cursor = conn.execute("""
        ...
        FROM people 
        WHERE is_active::integer = 1 AND meirav_code IS NOT NULL AND meirav_code != ''
        ORDER BY name
    """)
    people = cursor.fetchall()
